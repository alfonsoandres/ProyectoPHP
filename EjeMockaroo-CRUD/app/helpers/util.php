...
function funcionIpPais($ip){

   $paisCode = file_get_contents("http://ip-api.com/json/".$ip);
   $jsonDecode = json_decode($paisCode);
   
if ($jsonDecode->status=="success") {
    
    return strtolower( $jsonDecode->countryCode);
}
...
function crearPdf($cli){
    ...
    $pdf->Output('detallesCliente.pdf', 'I');
